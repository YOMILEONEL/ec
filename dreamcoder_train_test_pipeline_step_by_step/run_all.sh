...
# Compression/search hyperparameters. Defaults match the official list-domain
# paper experiments (see the repository's "official_experiments" file), not
# ecIterator's own defaults - notably ecIterator defaults to arity=0, which
# blocks consolidation from inventing any *parameterized* library function.
DREAMCODER_ARITY="${DREAMCODER_ARITY:-3}"
DREAMCODER_PSEUDOCOUNTS="${DREAMCODER_PSEUDOCOUNTS:-30}"
DREAMCODER_AIC="${DREAMCODER_AIC:-1.0}"
DREAMCODER_STRUCTURE_PENALTY="${DREAMCODER_STRUCTURE_PENALTY:-1.5}"
DREAMCODER_TOPK="${DREAMCODER_TOPK:-2}"
# Empty by default: falls back to enumerationTimeout/no step limit inside
# step04, matching the previous (undocumented) behavior. Set explicitly to
# give the recognition model its own training budget.
DREAMCODER_RECOGNITION_TIMEOUT="${DREAMCODER_RECOGNITION_TIMEOUT:-}"
DREAMCODER_RECOGNITION_STEPS="${DREAMCODER_RECOGNITION_STEPS:-}"
# step04 reads these hyperparameters straight from the environment
# (os.environ.get) rather than as positional CLI args, so they must be
# exported for the Python subprocess to see them.
export DREAMCODER_ARITY DREAMCODER_PSEUDOCOUNTS DREAMCODER_AIC \
       DREAMCODER_STRUCTURE_PENALTY DREAMCODER_TOPK \
       DREAMCODER_RECOGNITION_TIMEOUT DREAMCODER_RECOGNITION_STEPS
...
echo "DREAMCODER_ARITY=$DREAMCODER_ARITY"
echo "DREAMCODER_PSEUDOCOUNTS=$DREAMCODER_PSEUDOCOUNTS"
echo "DREAMCODER_AIC=$DREAMCODER_AIC"
echo "DREAMCODER_STRUCTURE_PENALTY=$DREAMCODER_STRUCTURE_PENALTY"
echo "DREAMCODER_TOPK=$DREAMCODER_TOPK"
echo "DREAMCODER_RECOGNITION_TIMEOUT=$DREAMCODER_RECOGNITION_TIMEOUT"
echo "DREAMCODER_RECOGNITION_STEPS=$DREAMCODER_RECOGNITION_STEPS"
echo "PYTHON=$PYTHON"
echo
